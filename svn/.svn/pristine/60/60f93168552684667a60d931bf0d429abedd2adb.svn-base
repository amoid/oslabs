...
Config* fetion_config_new()
{
	...
	config = (Config*)sal_malloc(sizeof(Config));
...
FxList* fetion_config_get_phrase(Config* config)
{
	...
	for(i = 0; i < nrows; i ++){
		phrase = (Phrase*)sal_malloc(sizeof(Phrase));
		start = ncols + i * ncols;
		...
		const char *sid, int laststate , int islastuser)
{
	...
	ul = (struct userlist*)sal_malloc(sizeof(struct userlist));
...
Proxy* fetion_config_load_proxy()
{
	...
	proxy = (Proxy*)sal_malloc(sizeof(Proxy));
...
char* xml_convert(xmlChar* in)
{
	...
	res = (char*)sal_malloc(strlen(pos) + 1);
