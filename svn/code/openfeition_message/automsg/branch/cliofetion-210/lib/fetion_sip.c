 ...
 *   levin108@gmail.com                                                    *
 *                                                                         *
 *   This program is sal_free software; you can redistribute it and/or modify  *
...
{
	FetionSip* sip = (FetionSip*)sal_malloc(sizeof(FetionSip));
...
{
	FetionSip* res = (FetionSip*)sal_malloc(sizeof(FetionSip));
...
{
	SipHeader* header = (SipHeader*)sal_malloc(sizeof(SipHeader));
	memset(header , 0 , sizeof(SipHeader));
	strcpy(header->name , name);
	header->value = (char*)sal_malloc(strlen(value) + 2 );
...
SipHeader* fetion_sip_authentication_header_new(const char* response)
{
	...
	res = (char*)sal_malloc(len);
	memset(res , 0 , len);
	sprintf(res , "%s%s%s" , start , response , end);
	header = (SipHeader*)sal_malloc(sizeof(SipHeader));
	memset(header , 0 , sizeof(SipHeader));
...
{
	SipMsg *msg = (SipMsg*)sal_malloc(sizeof(SipMsg));
...
{
	sipmsg->message = (char*)sal_malloc(n + 1);
...
void fetion_sip_message_sal_free(SipMsg* msg)
{
	SipMsg* pos = msg;
	SipMsg* pot;
	while(pos != NULL)
...
void fetion_sip_parse_notification(const char* sip , int* type , int* event , char** xml)
{
	...
	pos = strstr(sip , "\r\n\r\n") + 4;
	*xml = (char*)sal_malloc(strlen(pos) + 1);
...
void fetion_sip_parse_userleft(const char* sipmsg , char** sipuri)
{
	...
	res = xmlGetProp(node , BAD_CAST "uri");
	*sipuri = (char*)sal_malloc(xmlStrlen(res) + 1);
...
static char *generate_action_accept_body(Share *share)
{
	...
	{
	char *p = hexip_to_dotip("3B408066");
...
void fetion_sip_parse_sysmsg(const char* sipmsg , int *type
	...
	*showonce = atoi((char*)res);
	...
	res = xmlNodeGetContent(node);
	*content = (char*)sal_malloc(xmlStrlen(res) + 1);
	memset(*content, 0, xmlStrlen(res) + 1);
	strcpy(*content , (char*)res);
	xmlFree(res);
	node = node->next->next;
	res = xmlNodeGetContent(node);
	*url = (char*)sal_malloc(xmlStrlen(res) + 1);
...
void fetion_sip_get_auth_attr(const char* auth , char** ipaddress , int* port , char** credential)
{
	...
	char port_str[6] = { 0 };
	*credential = (char*)sal_malloc(256);
	memset(*credential , 0 , 256);
	*ipaddress = (char*)sal_malloc(256);
