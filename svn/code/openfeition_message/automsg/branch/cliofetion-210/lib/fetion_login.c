		...
		, const char* password , const char* publickey , const char* key)
{
	...
	nonce = (unsigned char*)sal_malloc(strlen(nouce) + 1);
	...
	res = (unsigned char*)sal_malloc(nonce_len + aeskey_len + psd_len + 1);
...
//	RSA_print_fp(stdout, r, 5);
	flen = RSA_size(r);
	out =  (unsigned char*)sal_malloc(flen);
	memset(out , 0 , flen);
...
void parse_sipc_reg_response(const char* reg_response , char** nouce , char** key)
{
	...
	n = strlen(pos) - strlen(strstr(pos , "\","));
	*nouce = (char*)sal_malloc(n + 1);
	strncpy(*nouce , pos , n);
	(*nouce)[n] = '\0';

	pos = strstr(pos , "key") + 5;
	n = strlen(pos) - strlen(strstr(pos , "\","));
	*key = (char*)sal_malloc(n + 1);
	strncpy(*key , pos , n);
	(*key)[n] = '\0';
...
int parse_sipc_auth_response(const char* auth_response , User* user, int *group_count, int *buddy_count)
{
	...
	if(xmlStrlen(buf) > 0){
		user->customConfig = sal_malloc(strlen((char*)buf) + 1);
...
static unsigned char* strtohex(const char* in , int* len) 
{
	unsigned char* out = (unsigned char*)sal_malloc(strlen(in)/2 );
...
static char* hextostr(const unsigned char* in , int len) 
{
	char* res = (char*)sal_malloc(len * 2 + 1);
	int i = 0;
...
static char* hash_password_v1(const unsigned char* b0 , int b0len , const unsigned char* password , int psdlen) 
{
	unsigned char* dst = (unsigned char*)sal_malloc(b0len + psdlen + 1);
...
static char* hash_password_v4(const char* userid , const char* password)
{
	...
	char *res , *dst;
	unsigned char* udomain = (unsigned char*)sal_malloc(strlen(domain));
	unsigned char* upassword = (unsigned char*)sal_malloc(strlen(password));
...
static char* generate_cnouce()
{
	char* cnouce = (char*)sal_malloc(33);
	memset( cnouce , 0 , 33 );
...
static unsigned char* decode_base64(const char* in , int* len)
{
	...
	if(len != NULL)
		*len = n;
	res = (unsigned char*)sal_malloc(n);
	memset(res , 0 , n);
	while(1)
	{
		memset(inp , 0 , 4);
