...
void fetion_contact_list_sal_free(Contact* contact)
{
...
int parse_add_buddy_verification(User* user , const char* str)
{
	...
	ver = (Verification*)sal_malloc(sizeof(Verification));
	...
	ver->algorithm = (char*)sal_malloc(n + 1);
	memset(ver->algorithm, 0, n + 1);
	strncpy(ver->algorithm , xml , n);
	xml = strstr(w , "type=") + 6;
	n = strlen(xml) - strlen(strstr(xml , "\""));
	ver->type = (char*)sal_malloc(n + 1);
