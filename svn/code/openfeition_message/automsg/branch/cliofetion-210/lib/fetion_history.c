...
{
	History* history = (History*)sal_malloc(sizeof(History));
...
void fetion_history_message_sal_free(History* history)
{
	sal_free(history);
}
FetionHistory* fetion_history_new(User* user)
{
	...
	fhistory = (FetionHistory*)sal_malloc(sizeof(FetionHistory));
...
void fetion_history_sal_free(FetionHistory* fhistory)
{
	if(fhistory){
		if(fhistory->db)
		...
		const char* userid , int count)
{
	...
	for(i = 0; i < nrows; i ++){
		start = ncols + i * ncols;
		his = (History*)sal_malloc(sizeof(History));
