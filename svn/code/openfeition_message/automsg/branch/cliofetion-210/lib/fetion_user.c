...
{
	User* user = (User*)sal_malloc(sizeof(User));
...
{
	user->verification->code = (char*)sal_malloc(strlen(code) + 1);
...
{
	Group* list = (Group*)sal_malloc(sizeof(Group));
...
{
	Verification* ver = (Verification*)sal_malloc(sizeof(Verification));
...
void fetion_user_load(User *user)
{
	...
	user->customConfig = (char*)sal_malloc(strlen(sqlres[ncols+18])+1);
...
static void parse_set_moodphrase_response(User* user , const char* sipmsg)
{
	...
	sal_free(user->customConfig);
	user->customConfig = (char*)sal_malloc(strlen((char*)res) + 1);
