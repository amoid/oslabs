...
//		return NULL;
	...
	FetionConnection* conn = (FetionConnection*)sal_malloc(sizeof(FetionConnection));
	if(conn == NULL){
		return NULL;
	}
...
FetionConnection* tcp_connection_new_with_port(const int port)
{
	...
	conn = (FetionConnection*)sal_malloc(sizeof(FetionConnection));
	if(conn == NULL){
		close(sfd);
		return NULL;
	}
