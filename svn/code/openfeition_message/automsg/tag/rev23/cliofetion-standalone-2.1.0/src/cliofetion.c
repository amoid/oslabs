...
int main(int argc, char *argv[])
{
	...
	cdb_memcpy(c_group[0].db_doc_message,"abandon vt.放弃  abruptly ad.突然  absorb vt.使精神贯注  abstract a.抽象的",
		cdb_strlen("abandon vt.放弃  abruptly ad.突然  absorb vt.使精神贯注  abstract a.抽象的"));
	...
	for(i = 0; i < c_group_count; i++){
		...
		for(j = 0; j < c_group[i].count; j++){
			...
			try_count = 0;
			while(send_message("13472798633",c_group[i].user[j].phone_no, 
